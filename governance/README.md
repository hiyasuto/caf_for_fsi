...
本リポジトリは、**FISC安全対策基準（第13版）**に準拠した Azure ガバナンスベースラインの IaC（Infrastructure as Code）テンプレートです。AWS BLEA for FSI の CDK テンプレートに相当する Azure 版リファレンス実装として、日本の金融機関が Azure クラウドを安全に利用するための基盤を提供します。
