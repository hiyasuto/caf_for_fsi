...
> FISC実務基準（実72, 実73, 実73-1）＋ 統制基準（統5-4）＋ コンティンジェンシープラン手引書 → Azure Reliability Testing / Incident Management

## 概要

本ドキュメントは、パブリッククラウドサービスに起因する障害シナリオの策定、机上演習・実機演習の実施、およびインシデント対応計画・コンティンジェンシープランの整備・更新に関するガイダンスです。

FISC安全対策基準（第13版）および金融庁「オペレーショナル・レジリエンス確保に向けた基本的な考え方」の要件を、Azure WAF の **Reliability Testing Strategy（RE:08）** および **Incident Management Plan（OE:08）** に基づいて実現します。

### 対応 FISC 基準

| 基準番号 | 基準名 | 本ドキュメントの対応箇所 |
|---------|--------|----------------------|
| 統5-4 | サイバーセキュリティに関する演習・訓練 | セクション 3, 4 |
| 実72 | 障害時の復旧テスト | セクション 2, 3 |
| 実73 | コンティンジェンシープランの策定 | セクション 5 |
| 実73-1 | サイバー攻撃想定のインシデント対応計画 | セクション 4, 5 |
...
Azure WAF Incident Management Plan に基づく、FISC 準拠のインシデント対応計画を整備します。

### 4.1 インシデント分類・重大度レベル

| 重大度 | 分類 | 影響 | 対応体制 | 通知先 |
|-------|------|------|---------|--------|
| **Critical** | 重要業務の全面停止 | 複数の重要業務が停止、顧客影響甚大 | 経営層＋全チーム緊急招集 | 金融庁、経営層、顧客、報道 |
| **High** | 重要業務の一部停止 | 特定の業務が停止、縮退運転可能 | インシデントマネージャー＋関連チーム | 経営層、顧客 |
| **Medium** | 業務影響軽微 | パフォーマンス低下、代替手段あり | 運用チーム | IT管理者 |
| **Low** | 業務影響なし | 一時的なエラー、自動回復 | 監視チーム | 記録のみ |
...
│  └──────┘   └──────┘   └──────┘   └──────┘   └──────┘  │
│                                                          │
│  Azure      Sentinel   Network    ASR/Backup  Azure      │
│  Monitor    Incident   Isolation  Failover    DevOps     │
│  Defender   Severity   NSG Rules  PITR        Retrospect │
│  XDR        Assignment            Rollback    ive        │
│                                                          │
│  ◄──── 自動化（Sentinel Playbook / Logic Apps）────►     │
└──────────────────────────────────────────────────────────┘
```

### 4.3 Azure を活用したインシデント対応体制

| フェーズ | Azureサービス | FISC 対応 | 説明 |
|---------|-------------|----------|------|
| **検知** | Azure Monitor アラート | 実59 | 障害検知・自動通知 |
| **検知** | Microsoft Sentinel | 実73-1 | セキュリティイベントの相関分析 |
| **検知** | Microsoft Defender XDR | 実14-1 | サイバー攻撃の端緒検知 |
| **トリアージ** | Sentinel インシデント管理 | 実73-1 | 重大度分類・担当者アサイン |
| **封じ込め** | NSG / Azure Firewall | 実73-1 | ネットワーク隔離・トラフィック遮断 |
| **封じ込め** | Entra ID Conditional Access | 実73-1 | 不正アクセスの即座のブロック |
| **復旧** | Azure Site Recovery | 実71 | DR サイトへのフェイルオーバー |
| **復旧** | Azure Backup（PITR） | 実39 | ポイントインタイムリストア |
| **復旧** | Azure Pipelines | 実72 | ロールバックデプロイ |
| **事後** | Azure DevOps（Work Items） | 実59 | 改善アクションの追跡 |
| **証拠保全** | Immutable Storage / Disk Snapshot | 実73-1 | フォレンジック用の証拠保全 |
...
| 手順 | 内容 | Azure 機能 |
|------|------|-----------|
| Break Glass アカウント維持 | 緊急用の管理者アカウントを複数保持 | Entra ID（MFA 除外、PIM 除外） |
| 定期的な動作確認 | 四半期ごとにアカウントの有効性を検証 | Entra ID Sign-in Logs |
| 使用ログの監視 | Break Glass アカウントの使用を即時検知 | Microsoft Sentinel アラートルール |
| 手順書の保管 | オフライン（紙媒体含む）での手順書保管 | — |

---

## 5. コンティンジェンシープランの整備・更新

FISCコンティンジェンシープラン策定手引書（第5版）のプロセスに基づき、演習結果をプランに反映する PDCA サイクルを定義します。

### 5.1 コンティンジェンシープラン更新サイクル
...
│    └─────────┘        └──────────┘                       │
│                                                          │
│  Plan: シナリオ策定 → 対応手順定義 → RTO/RPO設定          │
│  Do:   L1/L2/L3 演習の実施                               │
│  Check: RTO/RPO実績 vs 目標、課題・ギャップの特定          │
│  Act:  手順書更新、アーキテクチャ改善、次回演習計画        │
└──────────────────────────────────────────────────────────┘
```

### 5.2 更新トリガーと更新内容

| トリガー | 更新内容 | 対応 FISC 基準 |
|---------|---------|---------------|
| 演習結果（L1/L2/L3） | RTO/RPO 未達時の手順改善、エスカレーションフロー見直し | 実72, 実73 |
| Azure サービスの変更 | 新サービス・機能の活用、廃止サービスの代替手段 | 実73 |
| インシデント発生（実障害） | 根本原因分析に基づくプラン修正 | 実59, 実73 |
| 脅威情報の変化 | 新たなサイバー攻撃手法への対応シナリオ追加 | 統5-4, 実73-1 |
| アーキテクチャ変更 | ワークロード追加・変更に伴うDR設計の見直し | 実73 |
| 規制要件の変更 | FISC 基準改訂、金融庁ガイドライン改訂への対応 | 実73 |

### 5.3 更新プロセスにおける Azure 活用

| プロセス | Azureサービス | 説明 |
|---------|-------------|------|
| プラン文書管理 | Azure DevOps Wiki / SharePoint | バージョン管理・アクセス制御・承認ワークフロー |
| 改善アクション管理 | Azure DevOps Work Items | 演習で特定された課題の追跡・進捗管理 |
| 構成変更の反映 | Bicep / Terraform（IaC） | DR 構成の変更をコードとして管理・テスト |
| 自動検証 | Azure Chaos Studio（定期実験） | コンティンジェンシープランの有効性を定期的に自動検証 |
| リスク評価の更新 | Microsoft Defender for Cloud | セキュアスコア・推奨事項に基づくリスク再評価 |

### 5.4 プラン文書の構成

コンティンジェンシープランに含めるべき項目と、各項目の Azure による裏付けを示します。

| プラン項目 | 内容 | Azure による裏付け |
|-----------|------|-------------------|
| 対象システム一覧 | 重要業務とそれを支える Azure リソース | Azure Resource Graph によるリソースインベントリ |
| RTO/RPO 目標 | システムごとの復旧目標 | ASR / Azure Backup の構成で担保 |
| 障害シナリオ | セクション1のシナリオ一覧 | Azure Chaos Studio の実験定義 |
| 対応手順（Runbook） | 障害種別ごとの復旧手順 | Azure Automation Runbook |
| エスカレーションフロー | 連絡体制・意思決定者 | Azure Monitor アクショングループ |
| 通信手段 | 主系・代替系の通信手段 | Microsoft Teams / 電話 / SMS |
| 演習計画 | 年間演習スケジュール | Azure DevOps（スプリント計画） |
| 直近の演習結果 | 最新の演習結果・改善状況 | Azure DevOps Work Items |
...
- [Azure Cloud Adoption Framework — Secure](https://learn.microsoft.com/azure/cloud-adoption-framework/secure/)
...
| # | 次のドキュメント | 概要 |
|---|----------------|------|
| → | [04. 信頼性・事業継続](04-reliability.md) | バックアップ・DR設計の詳細 |
| → | [05. 運用管理](05-operations.md) | 障害記録・報告の運用プロセス |
| → | [サイバーレジリエンス ランディングゾーン](../landing-zone/cyber-resilience.md) | サイバー攻撃からの防御・検知・復旧の詳細設計 |
| → | [Azure Policy × FISCマッピング](../mapping/azure-policy-fisc-mapping.md) | ガードレールによる自動統制 |

---

*本ドキュメントは FISC安全対策基準・解説書 第13版（2025年3月）、FISCコンティンジェンシープラン策定手引書 第5版（2025年3月）、および Azure Well-Architected Framework Reliability Testing Strategy (RE:08) / Incident Management Plan (OE:08) に基づいています。*
