...
- 本アーキテクチャは [Microsoft Foundry](https://learn.microsoft.com/azure/ai-foundry/what-is-foundry) を中核プラットフォームとし、[Foundry Agent Service](https://learn.microsoft.com/azure/ai-foundry/agents/overview) によるエージェント運用基盤を構築します。
...
| ノーコード/ローコード AI アプリ構築 | Microsoft Copilot Studio | Power Platform 統合、既存業務フローへの組み込み、Entra ID認証 |
...
生成AI/エージェント基盤は用途によりTier 2〜3に位置づけられ、FISC第13版で新設された **AI安全管理基準（実150〜実153）** が全面的に適用されます。AIエージェントの導入にあたっては、自律的に業務判断を行うエージェントに対する統制・監査の仕組みが求められます。
...
### Microsoft Foundry を中核としたエージェントプラットフォーム
...
Microsoft Foundry（新）ポータルでは以下の拡張機能が利用可能です：

| 機能 | 説明 |
|------|------|
| **Foundry IQ（ナレッジ統合）** | エージェントをナレッジベースに接続し、企業・Web コンテンツに基づいた引用付き回答を生成。Azure AI Search の RAG を補完 |
| **Agent Memory** | エージェントが対話を跨いでコンテキスト情報を保持・想起。繰り返し入力を不要にし、パーソナライズされた応答を実現 |
| **Foundry Tool Catalog** | 1,400超のツール（MCP Server含む）を発見・構成・管理するカタログ。パブリック/プライベートカタログに対応 |
| **Model Router** | プロンプトに応じて最適なモデルを自動選択するルーティング機能（GA）。品質/コスト最適化のプロファイル設定が可能 |
| **マルチエージェント ワークフロー** | C# / Python SDK によるエージェント間の協調動作と複雑なワークフロー実行 |
| **M365/Teams/BizChat 公開** | Foundry から Microsoft 365、Teams、BizChat に直接エージェントを公開。コンテナー化デプロイにも対応 |
| **一元化 AI 資産管理（Operate）** | エージェント・モデル・ツールの 100% を一元管理。他クラウドのエージェント登録、アラート、フリート管理 |

> **参考**: [What is Microsoft Foundry?](https://learn.microsoft.com/azure/ai-foundry/what-is-foundry) / [What is Foundry Agent Service?](https://learn.microsoft.com/azure/ai-foundry/agents/overview)

### MCP / A2A / Tool Catalog によるエージェント連携

**Model Context Protocol（MCP）** と **Agent-to-Agent（A2A）Protocol** の2つのオープンプロトコルを活用し、エージェントの外部ツール接続と多エージェント間連携を実現します。**Foundry Tool Catalog** により 1,400 超のツール（MCP Server 含む）を検索・構成・管理でき、パブリックカタログとプライベートカタログの両方に対応しています。

| プロトコル / 機能 | 役割 | 金融機関での活用 |
|-----------|------|---------------|
...
| **Tool Catalog** | 1,400超のツールの検索・構成・管理 | 承認済みツールのプライベートカタログ管理、自社 API のツール化と社内共有 |
| **Microsoft Fabric Tool** | Fabric データとのエージェント統合 | 基幹データウェアハウス・レイクハウスへのエージェントアクセス |

MCP Server は Agent Identity Token（Entra Agent ID）で認証し、各エージェントが必要最小限のリソースにのみアクセスできるよう制御します。

> **参考**: [Model Context Protocol (MCP)](https://learn.microsoft.com/azure/ai-foundry/agents/how-to/tools/model-context-protocol) / [Agent-to-Agent (A2A)](https://learn.microsoft.com/azure/ai-foundry/agents/how-to/tools/agent-to-agent) / [Tool Catalog](https://learn.microsoft.com/azure/ai-foundry/agents/concepts/tool-catalog) / [Fabric Tool](https://learn.microsoft.com/azure/ai-foundry/agents/how-to/tools/fabric)

### Foundry IQ（ナレッジ統合）

**Foundry IQ** はエージェントをエンタープライズナレッジベースに接続し、企業データ・Web コンテンツに基づいた引用付き応答を生成します。Azure AI Search による RAG パイプラインを補完し、シンプルな設定で金融規制文書・社内マニュアル・FAQ などの知識ソースをエージェントに統合できます。

| 特徴 | 説明 |
|------|------|
| **エンタープライズナレッジ** | SharePoint、OneDrive、社内文書を知識ソースとして接続 |
| **Web 知識** | 公開 Web サイトの情報をエージェントの応答に活用 |
| **引用付き応答** | 回答の根拠となるソースを引用として提示（ハルシネーション抑制） |
| **RAG との棲み分け** | 大規模・高度なベクトル検索は Azure AI Search RAG、手軽な知識統合は Foundry IQ |

> **参考**: [What is Foundry IQ?](https://learn.microsoft.com/azure/ai-foundry/agents/concepts/what-is-foundry-iq)

### Agent Memory（エージェント記憶）

**Agent Memory** により、エージェントが対話を跨いでコンテキスト情報を保持・想起します。繰り返しの入力を不要にし、ユーザーごとにパーソナライズされた応答を実現します。

| 記憶タイプ | 説明 | 金融機関での活用例 |
|-----------|------|----------------|
| **ユーザー記憶** | 個々のユーザーの過去の対話や設定を記憶 | 顧客の取引傾向・好みの通貨ペアの記憶 |
| **共有記憶** | 組織全体で共有されるナレッジの蓄積 | 社内業務手続き・規制解釈の蓄積 |

> **参考**: [What is Agent Memory?](https://learn.microsoft.com/azure/ai-foundry/agents/concepts/what-is-memory)
...
**Microsoft Agent 365** を企業全体のAIエージェント統制プレーン（コントロールプレーン）として活用し、Copilot Studio・Microsoft Foundry・サードパーティツールで作成されたすべてのエージェントを一元管理します。
...
- **M365/Teams/BizChat 公開**: Foundry Agent Service から直接 Microsoft 365、Teams、Business Chat にエージェントを公開可能（コンテナー化デプロイにも対応）
- **一元化 Operate**: Microsoft Foundry（新）の Operate セクションにて、全エージェント・モデル・ツールのフリート管理、他クラウドのエージェント登録、アラート設定を一元化

> **参考**: [Publish agents to Microsoft 365](https://learn.microsoft.com/azure/ai-foundry/agents/how-to/publish-copilot)

### 閉域ネットワーク構成

金融機関のネットワークセキュリティ要件に対応するため、Microsoft Foundry の **Bring Your Own VNet（BYO VNet）** モデルを採用し、エンドツーエンドの閉域ネットワーク構成とします。
...
### マルチモデル対応（Microsoft Foundry Model Catalog）

Microsoft Foundry は **マルチモデルプラットフォーム** であり、ユースケースに応じて最適なモデルを選択できます。**Model Router**（GA）を利用すると、プロンプトごとに最適なモデルを自動選択し、品質とコストのバランスを最適化できます。以下のモデルが Azure から直接利用可能です。

| プロバイダー | 主なモデル | 特徴 |
|------------|----------|------|
| **OpenAI** | GPT-5.3-codex, GPT-5.2, GPT-5.1, GPT-5, GPT-4.1, GPT-4o, o-series | Azure OpenAI Service として提供。日本リージョン対応。Global Standard / Data Zone Standard / Provisioned 等のデプロイ方式 |
| **xAI** | Grok-4, Grok-4-fast, Grok-3, Grok-3-mini, Grok-code-fast | 高度な推論・コーディング能力。Data Zone Standard デプロイ |
| **DeepSeek** | DeepSeek-R1-0528, DeepSeek-V3.2, DeepSeek-V3.1 | オープンウェイト推論モデル。Japan East 対応 |
| **Anthropic** | Claude Opus 4.6, Claude Sonnet 4.6/4.5, Claude Haiku 4.5 | Marketplace 経由（パートナー提供）。Entra ID 認証対応 |
| **Meta** | Llama-4-Maverick, Llama-3.3-70B-Instruct | オープンウェイト。Global Provisioned Managed デプロイ |
| **Mistral** | Mistral-Large-3, Mistral-Document-AI | 文書理解に強み。Japan East 対応 |
| **Cohere** | Command-A, Embed-v4, Rerank-v4 | エンベディング・リランキングに最適 |
| **Moonshot AI** | Kimi-K2.5, Kimi-K2-Thinking | 高コンテキスト長推論 |
| **Microsoft** | Model Router (mai-model-router) | プロンプトに応じた最適モデル自動選択。品質/コスト最適化プロファイル対応 |

> **参考**: [Azure によって直接販売される Foundry Models](https://learn.microsoft.com/azure/ai-foundry/foundry-models/concepts/models-sold-directly-by-azure) / [パートナーおよびコミュニティの Foundry Models](https://learn.microsoft.com/azure/ai-foundry/foundry-models/concepts/models-from-partners) / [Model Router](https://learn.microsoft.com/azure/ai-foundry/openai/concepts/model-router)

金融機関のユースケースに応じたモデル選定の指針：

| ユースケース | 推奨モデル | 理由 |
|------------|----------|------|
| 汎用的な文書生成・Q&A | GPT-5.2 / GPT-5.1 | 高品質・多言語対応・Azure SLA |
| コーディング・IaC生成 | GPT-5.3-codex / Grok-code-fast | コード特化の最適化 |
| 高度な推論・分析 | o-series / Grok-4 / DeepSeek-R1 | 推論チェーン・数学的正確性 |
| 文書理解・OCR | Mistral-Document-AI | 構造化文書の解析に特化 |
| エンベディング・検索 | Cohere Embed-v4 / text-embedding-3 | RAG パイプラインの精度向上 |
| マルチモーダル分析 | GPT-5.2 / Claude Sonnet 4.6 | 画像・テキスト統合分析 |

### Microsoft Copilot Studio

**Microsoft Copilot Studio** は、ノーコード/ローコードでAIエージェントを構築・管理できるプラットフォームです。業務部門のユーザーが自らエージェントを作成でき、Power Platform との統合により既存業務フローへの組み込みが容易です。

| 機能 | 説明 |
|------|------|
| **ノーコード エージェント構築** | GUIベースでエージェントのトピック・アクション・ナレッジを定義 |
| **ナレッジ統合** | SharePoint、Dataverse、Webサイト等からの自動回答生成（生成AI活用） |
| **Power Platform 連携** | Power Automate フロー、Dataverse、Power BI との統合 |
| **マルチチャネル公開** | Teams、Web、モバイルアプリ、電話等への公開 |
| **Entra ID 認証** | エンタープライズ認証・条件付きアクセスポリシー準拠 |
| **DLP ポリシー** | Power Platform DLP によるコネクタ制限・データフロー制御 |
| **Financial Insights テンプレート** | 金融機関向けのエージェントテンプレート（文書分析スキル連携） |

金融機関での活用例：
- **社内ヘルプデスク**: 規程・マニュアルに基づく従業員向けQ&Aエージェント
- **顧客問い合わせ対応**: FAQ・商品情報に基づくチャットボット（Human-in-the-Loop付き）
- **業務フロー自動化**: 申請・承認プロセスの自動化エージェント（Power Automate連携）

> **参考**: [Microsoft Copilot Studio 概要](https://learn.microsoft.com/microsoft-copilot-studio/fundamentals-what-is-copilot-studio) / [Financial Insights エージェントテンプレート](https://learn.microsoft.com/microsoft-copilot-studio/template-financial-insights)
...
| コンポーネント | Azureサービス | 構成 | 理由 |
|-------------|-------------|------|------|
| 生成AI | Azure OpenAI Service | Private Endpoint、Japan East、複数デプロイ（GPT-5.x / o-series / Embedding） | 推論API、RAG、エージェントの推論エンジン |
| モデルルーター | Model Router (mai-model-router) | 品質/コスト最適化プロファイル、カスタムモデルサブセット | プロンプトに応じた最適モデル自動選択（GA） |
| マルチモデル | Microsoft Foundry Model Catalog | Grok / DeepSeek / Claude / Llama / Mistral 等（Marketplace含む） | ユースケース別の最適モデル選定 |
| ノーコード AI | Microsoft Copilot Studio | Entra ID認証、Power Platform DLP | 業務部門主導のエージェント構築・業務フロー自動化 |
| プロジェクト管理 | Microsoft Foundry | Hub + Project構成、BYO VNet | モデル評価・A/Bテスト・Red Teaming・デプロイ管理 |
| ナレッジ統合 | Foundry IQ | エンタープライズ/Web ナレッジソース接続 | Azure AI Search RAG を補完する手軽なナレッジ統合 |
| RAG検索 | Azure AI Search (Standard S2) | ベクトル検索 + Hybrid検索 + セマンティックランカー、Private Endpoint | 社内文書・規程・マニュアルのインデックス |
| データ統合 | Microsoft Fabric Tool | Agent Service ↔ Fabric 連携 | データウェアハウス・レイクハウスへのエージェントアクセス |
...
├──────────────────────────────────────────────────────────────────┤
│ Layer 4: LLM推論 (Azure OpenAI / Foundry Model Catalog)              │
│  ├─ システムプロンプトによる行動制約                                  │
│  ├─ RAGによるグラウンディング（Azure AI Search）                    │
│  ├─ マルチモデル選択（GPT-5.x / Grok / Claude / DeepSeek等）       │
...
| Agent Service DR | Foundry Agent Service の公式 DR ドキュメントに基づく復旧手順 |

> **参考**: [Agent Service Disaster Recovery](https://learn.microsoft.com/azure/ai-foundry/agents/concepts/agent-service-disaster-recovery) / [Operator DR](https://learn.microsoft.com/azure/ai-foundry/agents/concepts/agent-service-operator-disaster-recovery) / [Platform DR](https://learn.microsoft.com/azure/ai-foundry/agents/concepts/agent-service-platform-disaster-recovery)
...
| モデルデプロイ | Microsoft Foundry のモデルデプロイメント（Blue-Green対応） |
...
- [Foundry Models — Azure 直接販売モデル一覧](https://learn.microsoft.com/azure/ai-foundry/foundry-models/concepts/models-sold-directly-by-azure)
- [Foundry Models — パートナーおよびコミュニティモデル](https://learn.microsoft.com/azure/ai-foundry/foundry-models/concepts/models-from-partners)
- [Claude models in Microsoft Foundry](https://learn.microsoft.com/azure/ai-foundry/foundry-models/how-to/use-foundry-models-claude)
- [Microsoft Copilot Studio 概要](https://learn.microsoft.com/microsoft-copilot-studio/fundamentals-what-is-copilot-studio)
- [Financial Insights エージェントテンプレート](https://learn.microsoft.com/microsoft-copilot-studio/template-financial-insights)
